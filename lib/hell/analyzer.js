...
Analyzer.prototype.process = function(node) {
  switch (node.type) {
    ...
    case syntax.AssignmentExpression: return this.analyzeAssignment(node);
    case syntax.VariableDeclarator: return this.analyzeVarDeclarator(node);
  }
};
...
/**
 * Analyze AssignmentExpression.
 *
 * @param {Object} node
 * @api private
 */

Analyzer.prototype.analyzeAssignment = function(node) {
  this.add(0.3);
};

/**
 * Analyze VariableDeclarator.
 *
 * @param {Object} node
 * @api private
 */

Analyzer.prototype.analyzeVarDeclarator = function(node) {
  if (!node.init) return;
  this.add(0.3);
};
